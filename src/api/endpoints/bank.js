...
export const saveData = async (data, token) => {
    return await callPost(REQ.BANK.SAVE_DATA, data, token)
}

export const getData = async (id, token) => {
    return await callGet(`${REQ.BANK.GET_DATA}/${id}`, token)
}

export const updateBank = async (data, token) => {
    return await callPut(`${REQ.BANK.UPDATE_BANK}/${data.transactionId}`, data, token)
}
